...
import FlashThankYou from "./FlashThankYou";
...
const ContactForm = () =>
{
   ...
   const {name, email, reason, response} = state
    ...
    // Flash Message when true will display Flash Messsage
            ...
            <div className="flex flex-col items-center w-5/6 mx-auto my-12 fade-in-up  ">
                ...
                <form className="flex flex-wrap justify-start pt-8 md:grid md:grid-cols-2 md:row-gap-4 md:w-11/12 lg:w-3/4 xl:w-4/6  2xl:w-1/2" onSubmit={handleSubmit}>             
                   <label className="w-full pt-2 p-4 md:pt-4">Name
                    ...
                    <label className="w-full p-4 md:col-span-2">Message
                        <textarea className="w-full pb-8 pt-2 px-2 bg-transparent rounded border-2 placeholder-onyx resize-none lg:pb-20 focus:outline-none focus:border-accent" type="text" name="response" value={response} onChange={handleChange} placeholder='Enter Message' required/>
                    </label>
                ...
                <div className={!flashToggle ? "hidden" : "flex flex-col items-center w-full"}>
                    <FlashThankYou />
                </div>
            </div>
